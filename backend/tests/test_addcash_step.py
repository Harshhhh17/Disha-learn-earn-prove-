...
cmd = [r"C:\Program Files\Google\Chrome\Application\chrome.exe", "--headless=new", "--remote-debugging-port=9222", "--disable-gpu", "--no-sandbox", "--incognito", "http://localhost:3000"]
...
try:
    ...
    (async () => {
        try {
            ...
            // Execute onclick and catch any promise error
            let onclickError = null;
            try {
                await addBtn.onclick();
            } catch (e) {
                onclickError = { msg: e.message, stack: e.stack };
            }

            const modal = document.getElementById('razorpay-sandbox-modal');
            const toasts = Array.from(document.querySelectorAll('.toast')).map(t => t.innerText);
            ...
            return { topError: e.message, stack: e.stack };
        }
    })()
    """

    send_ws_json(s, {"id": 10, "method": "Runtime.evaluate", "params": {"expression": expr, "awaitPromise": True, "returnByValue": True}})
    s.settimeout(8.0)
    while True:
        msg = recv_ws_frame(s)
        if not msg: break
        if msg.get("id") == 10:
            print("STEP EVAL RESULT:", json.dumps(msg.get("result", {}), indent=2))
            break
finally:
    proc.kill()
